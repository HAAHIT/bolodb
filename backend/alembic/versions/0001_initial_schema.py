...
def upgrade() -> None:
    ...
    op.create_index("idx_query_history_conversation_id", "query_history", ["conversation_id"])
    op.create_index("idx_query_history_timestamp", "query_history", ["user_id", sa.text("timestamp DESC")])
    op.create_index("idx_conversations_user_id", "conversations", ["user_id", sa.text("updated_at DESC")])
