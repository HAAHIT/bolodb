...
async def refresh_jwt(refresh_token: str = Cookie(None)):
    ...
    try:
        ...
        new_token = app.controllers.auth.create_access_jwt(
...
async def me(user_token=Depends(get_current_user)):
    user = app.controllers.auth.get_me(user_token["user_id"])
...
async def login(request: Request):
    ...
    token = app.controllers.auth.login(body["email"], body["password"])
...
async def signup(request: Request):
    ...
    app.controllers.auth.signup(user_data)
...
async def change_password(request: Request, user_token=Depends(get_current_user)):
    body = await request.json()
    app.controllers.auth.change_password(
