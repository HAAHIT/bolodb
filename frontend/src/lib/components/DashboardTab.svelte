<script lang="ts">
  ...
  import type { HistoryStats, DbInfo } from '$lib/types';
  ...
  // Props are passed from the app shell; schema/stats are read from appState
  // so this stays in sync with the standalone /dashboard route.
  ...
  async function loadStats() {
    ...
    try {
      ...
      if (epoch === statsEpoch) statsLoading = false;
    }
  }
</script>
        ...
        <div class="card p-5 rise" style="animation-delay:0.15s;">
          ...
          <div style="display:flex;flex-direction:column;gap:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              ...
              <span style="font-size:14px;font-weight:700;color:var(--ink);">{dbInfo.dialect}</span>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;">
