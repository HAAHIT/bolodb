  ...
  onMount(() => {
    ...
    const typeInterval = setInterval(() => {
      ...
      if (i > fullDemoText.length) {
        ...
        setTimeout(() => { demoPhase = 1; }, 600);
        setTimeout(() => { demoPhase = 2; }, 1800);
        setTimeout(() => { demoPhase = 3; }, 2600);
        setTimeout(() => {
          demoPhase = 0;
          demoText = "";
          i = 0;
...
<svelte:head>
  <title>{$LL.common.appName()} — {$LL.common.tagline()}</title>
</svelte:head>
  ...
  <main class="flex-1 max-w-7xl mx-auto px-6 pt-32 pb-24 relative z-10 w-full">
    ...
    <div class="text-center max-w-4xl mx-auto mb-20 rise" style="animation-delay: 0.1s;">
      <h1 class="text-5xl md:text-7xl font-extrabold tracking-tight mb-6" style="color: var(--ink); line-height: 1.1;">
        {$LL.landing.talkToYourDatabase()} <br/><span style="color: var(--brand);">{$LL.landing.likeAHuman()}</span>
      </h1>
      <p class="text-xl md:text-2xl mb-10 font-medium max-w-2xl mx-auto" style="color: var(--muted);">
        {$LL.landing.noSqlRequired()}
      </p>
      <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
        <button class="btn btn-primary btn-lg shadow-xl hover:-translate-y-1 transition-transform w-full sm:w-auto" onclick={() => goto('/signup')}>{$LL.landing.startForFree()}</button>
        <button class="btn btn-ghost btn-lg w-full sm:w-auto" onclick={() => window.scrollTo({ top: 800, behavior: 'smooth' })}>{$LL.landing.viewDemo()}</button>
      </div>
    </div>
    ...
    <div class="card p-4 md:p-8 mb-24 max-w-5xl mx-auto rise shadow-2xl" style="animation-delay: 0.3s; backdrop-filter: blur(16px);">
      <div class="border rounded-xl overflow-hidden shadow-sm" style="border-color: var(--border); background: var(--bg);">
        <div class="px-4 py-3 border-b flex items-center gap-2" style="border-color: var(--border); background: var(--surface);">
          ...
          <span class="ml-4 text-xs font-mono" style="color: var(--muted);">{$LL.landing.demoTitle()}</span>
        </div>
...
LIMIT 3;</code></pre>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 rise" style="animation-delay: 0.5s;">
      <div class="card p-8 hover:-translate-y-1 transition-transform">
        ...
        <h3 class="text-xl font-bold mb-3" style="color: var(--ink);">{$LL.landing.featureZeroSetupTitle()}</h3>
        <p style="color: var(--muted); line-height: 1.6;">{$LL.landing.featureZeroSetupDesc()}</p>
      </div>
      ...
      <div class="card p-8 hover:-translate-y-1 transition-transform">
        ...
        <h3 class="text-xl font-bold mb-3" style="color: var(--ink);">{$LL.landing.featureDataStaysYoursTitle()}</h3>
        <p style="color: var(--muted); line-height: 1.6;">{$LL.landing.featureDataStaysYoursDesc()}</p>
      </div>
      ...
      <div class="card p-8 hover:-translate-y-1 transition-transform">
        ...
        <h3 class="text-xl font-bold mb-3" style="color: var(--ink);">{$LL.landing.featureTrustEngineTitle()}</h3>
        <p style="color: var(--muted); line-height: 1.6;">{$LL.landing.featureTrustEngineDesc()}</p>
      </div>
    </div>
