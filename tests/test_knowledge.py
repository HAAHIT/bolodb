...
def test_near_duplicate_questions_are_not_stored_twice(kb):
    kb.add_verified(
        DB_ID, "How many orders were placed last month?", "SELECT COUNT(*) FROM orders"
    )
    kb.add_verified(
        DB_ID, "How many orders were placed last month", "SELECT COUNT(*) FROM orders"
    )
...
def test_distinct_questions_are_both_stored(kb):
    ...
    kb.add_verified(
        DB_ID, "What is our total revenue this year?", "SELECT SUM(total) FROM orders"
    )
...
def test_retrieve_similar_ranks_closest_first(kb):
    ...
    kb.add_verified(
        DB_ID, "What is our total revenue?", "SELECT SUM(total) FROM orders"
    )
...
def test_retrieve_similar_respects_threshold_and_k(kb):
    ...
    results = kb.retrieve_similar(
        DB_ID, "Completely unrelated gibberish xyzzy", k=3, threshold=0.25
    )
    assert results == []
...
def test_retrieve_similar_is_scoped_to_db_id(kb):
    kb.add_verified(
        "db_a", "How many orders this month?", "SELECT COUNT(*) FROM orders"
    )
...
def test_glossary_round_trip_replaces_existing(kb):
    kb.set_glossary(
        DB_ID, [{"term": "active", "maps_to": "status = 'active'", "sql_hint": ""}]
    )
    kb.set_glossary(
        DB_ID, [{"term": "churned", "maps_to": "status = 'cancelled'", "sql_hint": ""}]
    )
